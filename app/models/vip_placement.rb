class VipPlacement < Service

  @expire_periud = 1.month
  @price = 1000
  @type_service = 'VipPlacement'
  ...
  def self.activated(user)
    self.where("status = ? AND expire_date < ? AND user_id = ?" ,Service::STATUS[:active], Date.now, user.id).size > 0 ? true : false
  end
