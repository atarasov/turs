...
function upload_callback(channel){
    ...
    $.ajax({type: "GET", url: "/attachments/manage?media=videos&page=1", dataType: "script"});
}
...
function upload_media_path() {
  ...
  to_path = "/attachments/create?media=videos";
...
function getType(v) {
	...
	// Google videos
	if (v.indexOf('http://videos.google.com/videoplay?docid=') == 0) {
		f.width.value = '425';
		f.height.value = '326';
		f.src.value = 'http://videos.google.com/googleplayer.swf?docId=' + v.substring('http://videos.google.com/videoplay?docid='.length) + '&hl=en';
		return 'flash';
	}
